...
file_name = "test_compare_transparent_falcon1024.csv"
...
def test_generateKeyPair():
    ...
    keys = generateKeyPair(1024) # 1024-bit Falcon algorithms is used in this project
    end = perf_counter_ns()
    return end - start
...
def test_verify_recoverable(keypair):
    ...
    signature = private_key.sign_recoverable(b"Hello World!")
    start = perf_counter_ns()
    result = public_key.verify_recoverable(b"Hello World!", signature, public_key.pubkey)
    end = perf_counter_ns()
    return end - start
...
if __name__ == "__main__":
    ...
    sign_time = 0
    for i in range(0, benchmark_iterations+1):
        new_benchmark = test_sign(generateKeyPair(1024))
        if i != 0:
    ...
    sign_r_time = 0
    for i in range(0, benchmark_iterations+1):
        new_benchmark = test_sign_recoverable(generateKeyPair(1024))
        if i != 0:
    ...
    recover_time = 0
    for i in range(0, benchmark_iterations+1):
        new_benchmark = test_recover(generateKeyPair(1024))
        if i != 0:
    ...
    verify_time = 0
    for i in range(0, benchmark_iterations+1):
        new_benchmark = test_verify(generateKeyPair(1024))
        if i != 0:
    ...
    verify_r_time = 0
    for i in range(0, benchmark_iterations+1):
        new_benchmark = test_verify_recoverable(generateKeyPair(1024))
        if i != 0:
