...
from parameters import falcon512_sampling_size, falcon1024_sampling_size
...
# Amount of created recoverable signatures to avoid rejection sampling.
recoverable_sig_samples = 3
...
class SecretKey:
    ...
    def __init__(self, n, polys=None):
        ...
        if (n == 512):
            self.recoverable_sig_samples = falcon512_sampling_size
        elif (n == 1024):
            self.recoverable_sig_samples = falcon1024_sampling_size
    ...
    def sign_recoverable(self, message, randombytes=urandom, hashfunc=SHAKE256):
        neutral_polynomial = [1]
        neutral_polynomial.extend((self.n - 1) * [0])
        int_header = 0x30 + logn[self.n]
        header = int_header.to_bytes(1, "little")
        salt = randombytes(SALT_LEN)
        hashed = self.hash_to_point(message, salt, hashfunc)
        ...
        # Creating multiple signatures (variable: recoverable_sig_samples) everytime
        # as a countermeasure for timing attacks based on rejection sampling.
        for j in range(0, self.recoverable_sig_samples):
            if randombytes == urandom:
                s_cache = self.sample_preimage(hashed)
            else:
                seed = randombytes(SEED_LEN)
                s_cache = self.sample_preimage(hashed, seed=seed)
            ...
            set_of_values.append(s_cache)

        # Set all value sets with an invertible s2 polynomial to an non-zero index
        # and the rest to the 0 index.
        target_indices = [-1] * (recoverable_sig_samples + 1)
        for h in range(1, recoverable_sig_samples + 1):
            new_index = floor(1 / (ntt(set_of_values[h-1][1]).count(0) + 1))
            target_indices[h * new_index] = set_of_values[h - 1]

        invertible_signatures = target_indices[1:]
        check_error = invertible_signatures.count(-1)
        indices = [i for i in range(0, len(invertible_signatures)) if invertible_signatures[i] != -1]

        
        if bool(recoverable_sig_samples - check_error):
            s = invertible_signatures[indices[0]]
            ...
            print("Try again, it has not been possible to create a correct signature!") # highly unlikely event
    ...
    def verify_recoverable(self, message, signature, pubkey):
        result = self.split_and_decompress_signature(signature)
        ...
        if norm_sign > self.signature_bound or pubkey != self.recover(message, signature):
            print("Squared norm of signature is too large:", norm_sign)
            return False
...
# Only used to determine the number of signature creations to lower chance of side-channel attacks
# and was utilized to create the benchmark "benchmark_datasets/test_compare_transparent_falcon.csv".
    def sign_recoverable_parameter(self, message, randombytes=urandom, hashfunc=SHAKE256):
        neutral_polynomial = [1]
        neutral_polynomial.extend((self.n - 1) * [0])
        int_header = 0x30 + logn[self.n]
        header = int_header.to_bytes(1, "little")
        ...
        while 1:
            ...
            if norm_sign <= self.signature_bound:
                ...
                enc_s2 = compress(s[1], self.sig_bytelen - HEAD_LEN - SALT_LEN)
                if enc_s1 is not False and enc_s2 is not False:
                    return ((header + salt + enc_s1 + enc_s2), counter)