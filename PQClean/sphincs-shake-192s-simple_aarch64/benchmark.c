...
int main(void) {
    ...
    for (int i = 0; i < count; i++) {
        int64_t begin, total;
        uint8_t *pk1 = calloc(1, PQCLEAN_SPHINCSSHAKE192SSIMPLE_AARCH64_CRYPTO_PUBLICKEYBYTES);
        uint8_t *sk1 = calloc(1, PQCLEAN_SPHINCSSHAKE192SSIMPLE_AARCH64_CRYPTO_SECRETKEYBYTES);
        PQCLEAN_SPHINCSSHAKE192SSIMPLE_AARCH64_crypto_sign_keypair(pk1, sk1);
        begin = gettime_i64();
    ...
    for (int i = 0; i < count; i++) {
        ...
        uint8_t *pk2 = calloc(1, PQCLEAN_SPHINCSSHAKE192SSIMPLE_AARCH64_CRYPTO_PUBLICKEYBYTES);
        uint8_t *sk2 = calloc(1, PQCLEAN_SPHINCSSHAKE192SSIMPLE_AARCH64_CRYPTO_SECRETKEYBYTES);
        PQCLEAN_SPHINCSSHAKE192SSIMPLE_AARCH64_crypto_sign_keypair(pk2, sk2);
        uint8_t *sig = calloc(1, PQCLEAN_SPHINCSSHAKE192SSIMPLE_AARCH64_CRYPTO_BYTES);
        size_t siglen;
        uint8_t m = 1;
        size_t mlen = sizeof(m);
        PQCLEAN_SPHINCSSHAKE192SSIMPLE_AARCH64_crypto_sign_signature(sig, &siglen, &m, mlen, sk2);
        begin = gettime_i64();
        for (int j = 0; j < iterations; j++){
            PQCLEAN_SPHINCSSHAKE192SSIMPLE_AARCH64_crypto_sign_signature(sig, &siglen, &m, mlen, sk2);
        }
        ...
        PQCLEAN_SPHINCSSHAKE192SSIMPLE_AARCH64_crypto_sign_verify(sig, siglen, &m, mlen, pk2);
        begin = gettime_i64();
        for (int j = 0; j < iterations; j++){
            PQCLEAN_SPHINCSSHAKE192SSIMPLE_AARCH64_crypto_sign_verify(sig, siglen, &m, mlen, pk2);
        }
