...
int main(void) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        uint8_t m = 1;
        size_t mlen = sizeof(m);
        PQCLEAN_FALCON1024_AARCH64_crypto_sign_signature_pkr(sig, &siglen, &m, mlen, sk3);
        begin = gettime_i64();
        for (int j = 0; j < iterations; j++){
            PQCLEAN_FALCON1024_AARCH64_crypto_sign_signature_pkr(sig, &siglen, &m, mlen, sk3);
        }
        ...
        PQCLEAN_FALCON1024_AARCH64_crypto_sign_verify_pkr_recover(sig, siglen, &m, mlen, pk3, pk_recovered);
        begin = gettime_i64();
        for (int j = 0; j < iterations; j++){
            PQCLEAN_FALCON1024_AARCH64_crypto_sign_verify_pkr_recover(sig, siglen, &m, mlen, pk3, pk_recovered);
        }
