...
do_sign_pkr(uint8_t *nonce, uint8_t *sigbuf, size_t *sigbuflen,
    ...
    /*
     * Compute and return the signature.
     * Error will not occur with a realistic probability
     * if PKR_SIG_SAMPLES is set correctly in the function "PQCLEAN_FALCON512_AARCH64_sign_dyn_pkr".
     */
    int error = PQCLEAN_FALCON1024_AARCH64_sign_dyn_pkr(r.sig, &sc, f, g, F, G, r.hm, tmp.b);
    v = PQCLEAN_FALCON1024_AARCH64_comp_encode_pkr(sigbuf, *sigbuflen, r.sig);
    if (v != 0 && !error) {
        inner_shake256_ctx_release(&sc);
        *sigbuflen = v;
        return 0;
...
do_verify_pkr_recover(
    const uint8_t *nonce, const uint8_t *sigbuf, size_t sigbuflen,
    const uint8_t *m, size_t mlen, uint8_t *pk, uint8_t *pk_rec) {
    union {
        uint8_t b[2 * FALCON_N];
        uint64_t dummy_u64;
        fpr dummy_fpr;
    ...
    v = PQCLEAN_FALCON1024_AARCH64_comp_decode_pkr(sig, sigbuf, sigbuflen);
    if (v == 0) {
        return -1;
    }
    if (v != sigbuflen) {
        if (sigbuflen == PQCLEAN_FALCONPADDED1024_AARCH64_CRYPTO_BYTES_PKR - NONCELEN - 1) {
            while (v < sigbuflen) {
                if (sigbuf[v++] != 0) {
                    return -1;
                }
    ...
    if (!PQCLEAN_FALCON1024_AARCH64_verify_recover(h, hm, sig + 1024, sig, tmp.b)) {
        return -1;
    }
     ...
     * Encode public key.
     */
    pk_rec[0] = 0x00 + FALCON_LOGN;
    v = PQCLEAN_FALCON1024_AARCH64_modq_encode(
            pk_rec + 1, PQCLEAN_FALCON1024_AARCH64_CRYPTO_PUBLICKEYBYTES - 1,
            h_unsigned, FALCON_LOGN);
    if (v != PQCLEAN_FALCON1024_AARCH64_CRYPTO_PUBLICKEYBYTES - 1) {
        return -1;
    }

    int diff = memcmp(pk, pk_rec, 512);
    if (diff != 0) {
        return -1;
    }
...
/* see api.h */
int
PQCLEAN_FALCON1024_AARCH64_crypto_sign_signature_pkr(
    uint8_t *sig, size_t *siglen,
    const uint8_t *m, size_t mlen, const uint8_t *sk) {
    size_t vlen;

    vlen = PQCLEAN_FALCON1024_AARCH64_CRYPTO_BYTES_PKR - NONCELEN - 1;
...
/* see api.h */
int
PQCLEAN_FALCON1024_AARCH64_crypto_sign_verify_pkr_recover(
    const uint8_t *sig, size_t siglen,
    const uint8_t *m, size_t mlen, uint8_t *pk, uint8_t *pk_rec) {
    if (siglen < 1 + NONCELEN) {
        return -1;
    ...
    return do_verify_pkr_recover(sig + 1,
                     sig + 1 + NONCELEN, siglen - 1 - NONCELEN, m, mlen, pk, pk_rec);
}
