...
do_sign_pkr(uint8_t *nonce, uint8_t *sigbuf, size_t *sigbuflen,
        const uint8_t *m, size_t mlen, const uint8_t *sk, int *counter) {
    union {
        uint8_t b[72 * FALCON_N];
        uint64_t dummy_u64;
        fpr dummy_fpr;
     ...
     */
    PQCLEAN_FALCON1024_AARCH64_sign_dyn_pkr(r.sig, &sc, f, g, F, G, r.hm, tmp.b, counter);
...
PQCLEAN_FALCON1024_AARCH64_crypto_sign_signature_pkr(
    ...
    if (do_sign_pkr(sig + 1, sig + 1 + NONCELEN, &vlen, m, mlen, sk, counter) < 0) {
        return -1;
    }
    sig[0] = 0x30 + FALCON_LOGN;
    *siglen = 1 + NONCELEN + vlen;
