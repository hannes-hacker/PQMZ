...
# Configuration of the access point GitHub repository to initialize to the branch
...
if [ $number_additions -eq 2 ]; then
  ...
  cd ..
  hash_curves_reference=$(cat secuer_ceremony_storage/hashes/curves.txt)
  hash_proofs_reference=$(cat secuer_ceremony_storage/hashes/proofs.txt)
...
# Here you could check for an verified hash over all directories and all files.
